...
export default function AdminFolderImageGallery({ folder, onImageUpdate }: AdminFolderImageGalleryProps) {
  ...
  const [editingOrientation, setEditingOrientation] = useState<{ [key: string]: number }>({});
  const [isSavingOrientation, setIsSavingOrientation] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    console.log('Image rotations:', folder.files.map(f => ({ id: f.id, rotation: f.rotation })));
  }, [folder]);
  ...
  const getImageUrl = (image: File) => {
    ...
    return thumbVariant?.path || webpVariant?.path || originalVariant?.path || '';
  };

  const getEffectiveOrientation = (image: File) => {
    return editingOrientation[image.id] !== undefined ? editingOrientation[image.id] : (image.rotation || 1);
  };
  ...
  const saveOrientation = async (imageId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setStatusMessage({ type: 'success', text: 'Orientation saved successfully' });
        onImageUpdate();
        setEditingOrientation(prev => {
          const newState = { ...prev };
  ...
  const handleDeleteSelected = async () => {
    if (selectedImageIds.size === 0) {
      setStatusMessage({ type: 'error', text: 'No images selected for deletion.' });
      return;
    }

    if (!confirm(`Are you sure you want to delete ${selectedImageIds.size} image(s)? This action cannot be undone.`)) {
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        setStatusMessage({ type: 'success', text: `${selectedImageIds.size} image(s) deleted successfully.` });
        setSelectedImageIds(new Set());
        onImageUpdate();
      } else {
        const errorData = await res.json();
        setStatusMessage({ type: 'error', text: `Failed to delete images: ${errorData.error || 'Unknown error'}` });
      }
    } catch (error) {
      console.error('Error deleting images:', error);
      setStatusMessage({ type: 'error', text: 'An unexpected error occurred during deletion.' });
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleOrientSelected = async () => {
    if (selectedImageIds.size === 0) {
      setStatusMessage({ type: 'error', text: 'No images selected for orientation change.' });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        setStatusMessage({ type: 'success', text: `${selectedImageIds.size} image(s) oriented successfully.` });
        onImageUpdate();
      } else {
        const errorData = await res.json();
        setStatusMessage({ type: 'error', text: `Failed to orient images: ${errorData.error || 'Unknown error'}` });
      }
    } catch (error) {
      console.error('Error orienting images:', error);
      setStatusMessage({ type: 'error', text: 'An unexpected error occurred during orientation.' });
    }
  finally {
  ...
  const getDisplayOrientation = (image: File) => {
    ...
    const degrees = exifOrientationToDegrees;
    ...
    <div className="mt-8">
      ...
      {images.length === 0 ? (
        <div className="text-center py-12 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
          ...
          <p className="text-gray-500 dark:text-gray-400">No images in this folder</p>
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Upload images using the form above</p>
        </div>
      ) : (
        <>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <select
                    ...
                    onChange={(e) => setOrientationValue(parseInt(e.target.value))}
          ...
          {viewMode === 'grid' ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {sortedImages.map((image) => {
                ...
                const hasOrientationEdit = editingOrientation[image.id] !== undefined;
                  ...
                  >
                    ...
                    >
                      <Image
                        ...
                        style={{
                          objectFit: 'contain',
                          transform: exifOrientationToCssTransform(currentOrientation),
                      ...
                      <div className={`absolute top-2 left-2 text-xs px-2 py-1 rounded-full capitalize ${
                    ...
                    <div className="p-3">
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        <select
                          ...
                          onChange={(e) => handleOrientationChange(image.id, parseInt(e.target.value))}
              ...
              <table className="w-full">
                <thead className="bg-gray-50 dark:bg-gray-800 border-b">
                  <tr>
                    ...
                    <th className="text-left p-3 text-sm font-medium">Display</th>
                    <th className="text-left p-3 text-sm font-medium">Orientation</th>
                  ...
                  {sortedImages.map((image) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-3">
                          <p className="font-medium text-sm" title={image.fileName}>
                            {image.fileName}
                          </p>
                        </td>
