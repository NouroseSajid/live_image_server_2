...
export default function AdminFolders() {
  ...
  const [uploadStatusMessage, setUploadStatusMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);
  ...
  const handleImageUpload = async (e: React.FormEvent) => {
    ...
    if (selectedFiles.length === 0 || !selectedFolderForUpload) {
      setUploadStatusMessage({ type: 'error', text: 'Please select files and a target folder.' });
      return;
    }
    ...
    if (errorCount === 0) {
      setUploadStatusMessage({ type: 'success', text: `${successCount} files uploaded successfully!` });
    } else {
      setUploadStatusMessage({ type: 'error', text: `Uploaded ${successCount} files, but ${errorCount} failed. See console for details.` });
    }
  ...
  const handleEditFolder = async (folder: Folder) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatusMessage({ type: 'error', text: `Failed to fetch folder details: ${errorText}` });
      }
    } catch (error) {
      console.error('Error fetching folder details:', error);
      setStatusMessage({ type: 'error', text: 'An unexpected error occurred while fetching folder details.' });
    }
  };
  ...
  return (
    <div className="admin-theme p-6" style={{ backgroundColor: 'var(--admin-background)', color: 'var(--admin-text)' }}>
          ...
          {folders.map((folder) => (
            <li key={folder.id} className="flex items-center justify-between gap-4 p-3 rounded" style={{ backgroundColor: 'var(--admin-card)' }}>
              ...
              <div className="flex items-center gap-2">
                <button className="px-2 py-1 rounded border" onClick={() => handleEditFolder(folder)} style={{ borderColor: 'var(--admin-border)' }}>
                  Edit
                </button>
                <button
      ...
      <section className="mb-8">
        ...
        {editingFolder && (
          <>
            {console.log('Rendering AdminFolderImageGallery with folder:', editingFolder)}
            <AdminFolderImageGallery folder={editingFolder} onImageUpdate={() => handleEditFolder(editingFolder)} />
          </>
        )}
      </section>
      ...
      <section className="mb-8">
        ...
        {uploadStatusMessage && (
          <div className={`mb-3 p-2 rounded ${uploadStatusMessage.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            {uploadStatusMessage.text}
          </div>
        )}
        <form onSubmit={handleImageUpload} className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl">
          <input
            type="file"
            multiple
          ...
          <div className="md:col-span-2">
            <button
              ...
              className="px-4 py-2 rounded"
              style={{ backgroundColor: 'var(--admin-primary)', color: 'var(--admin-primary-foreground)' }}
              disabled={uploading || selectedFiles.length === 0 || !selectedFolderForUpload}
            >
