...
export async function POST(request: NextRequest) {
  ...
  if (!folderId || !fileName) {
    return new NextResponse('Missing folderId or fileName in headers', { status: 400 });
  }
  ...
  try {
    ...
    const permanentFolderBase = path.join(process.cwd(), 'public', 'images', folderId);
    ...
    if (fileType === 'image') {
      ...
      await sharp(imageBuffer).resize(200, 200, { fit: 'inside' }).webp({ quality: 60 }).toFile(thumbPath);
    } else {
    ...
    const responseBody = JSON.stringify({ message: 'File uploaded successfully', file: newFile }, jsonReplacer);
    return new NextResponse(responseBody, { status: 201, headers: { 'Content-Type': 'application/json' } });

  } catch (error) {
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
      return new NextResponse('File with this name already exists in the folder', { status: 409 });
    }
