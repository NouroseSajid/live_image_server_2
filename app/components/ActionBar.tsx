...
export default function ActionBar({
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[60] animate-in fade-in slide-in-from-bottom-10 duration-500 ease-out w-full px-3 sm:w-auto">
      <div className="bg-zinc-900/90 border border-white/10 shadow-[0_30px_60px_rgba(0,0,0,0.5)] rounded-xl sm:rounded-[24px] px-3 py-2 sm:px-8 sm:py-5 flex flex-row items-center gap-3 sm:gap-8 backdrop-blur-2xl">
        <div className="flex items-center gap-2 sm:gap-3 sm:pr-8 sm:border-r sm:border-white/10">
          ...
          <div className="min-w-0">
            <span className="text-sm sm:text-sm font-bold block leading-none">
              Selected
            </span>
            <span className="text-[10px] sm:text-[10px] text-zinc-500 uppercase font-bold tracking-tighter break-words">
        ...
        <button
          type="button"
          onClick={onClear}
          className="ml-auto p-2 sm:p-3 hover:bg-white/5 rounded-full text-zinc-600 hover:text-white transition-colors flex-shrink-0"
